...
import utils
from utils import lookup_activation, apply_mixed_activations
...
class Objective:
    ...
    def _objective_error_transform(self, inp, action_diff=None, ftype=None):
        """
        ...
        if ftype == 'learned-reinforce-3factor-vector':
            error =  tf.reduce_mean(action_diff*tf.stop_gradient(self.obj_func(inp, action_diff.shape[-1], output_bias=use_error_func).out), axis=-1) * tf.get_variable('factor', [], tf.float32, initializer=tf.ones_initializer()) + 0*tf.squeeze(self.obj_func(inp, 1, output_bias=use_error_func).out, axis=-1)
        elif ftype == 'learned-reinforce-3factor-scalar':
            error = tf.squeeze(tf.stop_gradient(self.obj_func(inp, 1, output_bias=use_error_func).out), axis=-1) * tf.reduce_mean(action_diff, axis=-1) * tf.get_variable('factor', [], tf.float32, initializer=tf.ones_initializer()) + 0*tf.squeeze(self.obj_func(inp, 1, output_bias=use_error_func).out, axis=-1)
        else:
    ...
    def _create_objective(self, x, a, trans, seq_len, seq_mask, agent, policy, create_summary=False):
        ...
        if ftype == 'learned-reinforce' or ftype ==  'learned-reinforce-gradcritic' or ftype == 'learned-reinforce-3factor-scalar' or ftype == 'learned-reinforce-3factor-vector':
            ...
            #if ftype == 'learned-reinforce-3factor-scalar':
            #    action_error = tf.reduce_mean(((rb_action - policy.action) ** 2), axis=-1)
            ...
            '''
                ...
                return action_error * tf.get_variable('factor', [], tf.float32, initializer=tf.ones_initializer()) + 0*self._objective_error_transform(outputs)
            elif ftype == 'learned-reinforce-3factor-scalar':
                a_mod = self.obj_func(outputs, 1).out#mlp(outputs, 1, -1, -1, None, False).out
                action_error = tf.reduce_mean(tf.stop_gradient(a_mod) * ((rb_action - policy.action) ** 2), axis=-1)
                return action_error * tf.get_variable('factor', [], tf.float32, initializer=tf.ones_initializer()) + 0*self._objective_error_transform(outputs)
            else:
            '''
            return self._objective_error_transform(outputs, action_diff=(rb_action - policy.action) ** 2, ftype=ftype)
